      ...
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="grid md:grid-cols-2 gap-8 mb-16">
          {rituals.map((ritual, index) => (
            <div
              key={ritual?.id ?? ritual?.slug ?? ritual?.title ?? ritual?.name ?? index}
              className={`transition-all duration-700 transform `}
            >
              <div className="bg-white rounded-3xl p-6 sm:p-8 shadow-md sm:hover:shadow-xl transition-all duration-300 h-full">
                {/* Header */}

                <h3 className="text-2xl font-semibold text-primaryBlue mb-6">{ritual.title}</h3>
                ...
                <div>
                  ...
                  <ul className="space-y-2">
                    {ritual.tools.map((tool, toolIndex) => (
                      <li key={tool?.id ?? tool?.slug ?? tool?.title ?? tool?.name ?? toolIndex} className="flex items-center text-gray-600">
        ...
        <div className="bg-white rounded-3xl p-8 shadow-lg">
          <h3 className="text-2xl font-semibold text-gray-900 mb-6 text-center">Tool Transparency</h3>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center items-center px-4 sm:px-0 mx-auto mt-16">
            <a target="_blacnk" href="https://join.slack.com/t/shiftdeployworkspace/shared_invite/zt-3dej8l23q-m2S_MrlF7zvL2F~9jwK6iA"
              ...
              className="bg-primaryOrange text-white px-4 sm:px-6 lg:px-8 xl:px-10 py-2.5 sm:py-4 rounded-lg sm:rounded-xl lg:rounded-2xl font-semibold flex items-center justify-center gap-x-2 hover:bg-toOrange text-md"
            >
              Join Us On Slack 
            </a >
