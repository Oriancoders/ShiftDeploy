        ...
        <div className="grid lg:grid-cols-2 gap-12 mb-16 px-6 max-w-6xl mx-auto">
          ...
          >
            <div className="bg-white rounded-3xl p-6 sm:p-8 shadow-md h-full">
              <h3 className="text-xl sm:text-2xl font-semibold text-gray-900 mb-6">Approval Gates</h3>
              <ul className="space-y-4">
                {approvalGates.map((gate, index) => (
                  <li key={gate?.id ?? gate?.slug ?? gate?.title ?? gate?.name ?? index} className="flex items-start text-xs sm:text-sm">
          ...
          >
            <div className="bg-white rounded-3xl p-6 sm:p-8 shadow-md h-full">
              <h3 className="text-xl sm:text-2xl font-semibold text-primaryBlue mb-6">Your Control Points</h3>
              <ul className="space-y-4">
                {controlPoints.map((control, index) => (
                  <li key={control?.id ?? control?.slug ?? control?.title ?? control?.name ?? index} className="flex items-start  text-xs sm:text-sm">
                    <CheckCircle className="w-3 sm:w-4 h-3 sm:h-4 text-primaryOrange mr-3 mt-1 flex-shrink-0" />
                    <div>
        ...
        >
          <div className="sm:bg-white rounded-3xl p-6 sm:p-12 shadow-md text-center max-w-6xl mx-auto">
            <div className="flex justify-center mb-6">
              {[...Array(5)].map((_, i) => (
                <Star key={_?.id ?? _?.slug ?? _?.title ?? _?.name ?? i} className="size-6 text-yellow-500 fill-current" />
              ))}
            </div>
