...
export default function EssentialPlanCard({ isAnnual, onSelectPackage }) {
  ...
  return (
    ...
    >
      ...
      <div className="mb-8">
        <span className="text-4xl font-semibold text-[#0C1F3A]">${displayPrice}</span>
      ...
      <ul className="gap-y-4 mb-8 flex-grow">
        {essentialFeatures.map((feature, i) => (
          <li key={feature?.id ?? feature?.slug ?? feature?.title ?? feature?.name ?? i} className="flex font-medium text-gray-600 text-sm">
            <Check className="size-5 text-green-500 mr-3 flex-shrink-0" /> {feature}
          </li>
        ))}
      </ul>
