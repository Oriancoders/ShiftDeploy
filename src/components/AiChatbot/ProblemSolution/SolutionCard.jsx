...
export default function SolutionCard() {
  return (
    ...
    >
      <div className="absolute top-0 right-0 size-64 bg-gradient-to-br from-secondaryBlue to-primaryOrange blur-[80px] rounded-full opacity-30 pointer-events-none" />
      <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] opacity-5" />

      <div className="size-14 bg-gradient-to-br from-primaryOrange to-orange-600 rounded-2xl flex items-center justify-center mb-8 border border-orange-400 shadow-lg relative z-10">
      ...
      <ul className="gap-y-5 mb-10 flex-grow z-10 relative">
        {solutionItems.map((item, i) => (
          <motion.li
            key={item?.id ?? item?.slug ?? item?.title ?? item?.name ?? i}
          ...
          >
            <div className="size-6 rounded-full bg-green-500/20 border border-green-500/30 flex items-center justify-center mr-4 flex-shrink-0 mt-0.5 shadow-[0_0_10px_rgba(34,197,94,0.2)]">
